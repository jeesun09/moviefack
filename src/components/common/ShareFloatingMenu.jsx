...
export default function ShareFloatingMenu({ title = "Movie", url = "" }) {
  ...
  const [radialLayout, setRadialLayout] = useState("right");
  ...
  // Detect position relative to screen edges on open
  useEffect(() => {
    if (isOpen && menuRef.current) {
      ...
      if (rect.left < 90) {
        // Near left screen edge (common on mobile) -> Fan to the right & top
        setRadialLayout("right");
      } else if (screenWidth - rect.right < 90) {
        // Near right screen edge -> Fan to the left & top
        setRadialLayout("left");
      } else {
        // Centered -> Fan upward symmetrically
        setRadialLayout("center");
      }
    }
  }, [isOpen]);
  ...
  // Adaptive circular coordinates based on screen bounds
  const getCoordinates = (index) => {
    if (radialLayout === "right") {
      // Fan upwards & to the right (all x >= 0)
      const positions = [
        { x: 0, y: -64 },
        { x: 42, y: -48 },
        { x: 62, y: -5 },
    ...
    if (radialLayout === "left") {
      // Fan upwards & to the left (all x <= 0)
      const positions = [
        { x: -56, y: 26 },
        { x: -62, y: -16 },
        { x: -42, y: -48 },
